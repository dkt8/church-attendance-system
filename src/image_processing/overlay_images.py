...
def overlay_images(target, name, overlay_path, saint_name, full_name, x_position, y_position):
    ...
    draw.text((x_adjusted, y_adjusted),       saint_name, font=font, fill=(0, 0, 0, 255))  # Black text with full opacity
    ...
    if x_adjusted + text_width > background.width:
        ...
        x_adjusted = x_position - (text_width // 2)
        y_adjusted = y_position - (text_height // 2)    
        draw.text((x_adjusted, y_adjusted + 100), full_name, font=font, fill=(0, 0, 0, 255))  # Black text with full opacity
    else:
        draw.text((x_adjusted, y_adjusted + 100), full_name, font=font, fill=(0, 0, 0, 255))  # Black text with full opacity
